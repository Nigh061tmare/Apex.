...
function main() {
  ...
  const dbzEpsSrc = read('dragonball_dbz_episodes.json');
  const scenSrc = read('dragonball_scenarios.json');
  const relSrc = read('dragonball_relations.json');
  const libSrc = read('dragonball_character_library.json');
  ...
  const dbzEpisodes = (dbzEpsSrc.episodes || []).map((e) => ({
    n: e.order,
    jp: (e.jp || '').slice(0, 120),
    es: (e.es || '').slice(0, 120),
    s: (e.synopsis || '').slice(0, 360),
    p: e.page || null
  }));
  const dbzEpisodesMeta = dbzEpsSrc._meta || null;

  // --- Escenarios y geografia (Chozenshu 1, pp.244-258) ---
  const scenarios = (scenSrc.regions || []).map((r) => ({
    id: r.id, name: r.name, t: r.tomo,
    p: r.page || (r.pages && r.pages[0]) || null,
    terrain: (r.terrain || '').slice(0, 400),
    landmarks: r.landmarks || [],
    hook: r.engineHook || null
  }));

  // --- Relaciones canonicas (Chozenshu 1, p.10) ---
  const relations = (relSrc.relations || []).map((r) => ({
    a: r.from, b: r.to, k: r.type, kind: r.kind, note: (r.note || '').slice(0, 220), p: r.page
  }));
  const comboHints = (relSrc.comboHints || []).map((c) => ({
    name: c.name, members: c.members || [], note: (c.note || '').slice(0, 200)
  }));
  ...
  const codex = {
    ...
    gtEpisodes, gtEpisodesMeta, dbzEpisodes, dbzEpisodesMeta,
    scenarios, relations, comboHints, passives, passCoverage, byCharacter
  };
  ...
  const deep = {
    ...
    characterLibrary: (libSrc.pages || []).map((x) => ({
      p: x.p, names: x.names || [], x: (x.text || '').slice(0, 1200)
    }))
  };
  ...
  console.log(`  episodios DBZ   ${dbzEpisodes.length}`);
  console.log(`  escenarios      ${scenarios.length}`);
  console.log(`  relaciones      ${relations.length}`);
  console.log(`  biblioteca      ${(libSrc.pages || []).length} (bundle deep)`);
