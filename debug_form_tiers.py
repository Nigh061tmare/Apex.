...
targets = {"megumi-fushiguro", "akaza-kimetsu", "ant-man-marvel-616", "scarecrow-dc-rebirth", "dende-saga-namek-100"}
...
for i, line in enumerate(lines):
    ...
    if m:
        ...
        in_table = False
        continue
    m = re_meta.match(line) if 're_meta' in dir() else None
    if "- **Base Tier***" in line:
        # Simple parse
        import re as re2
        m2 = re2.search(r'Base Tier\*\*:\s*`([^`]+)`\s+\|\s+\*\*Base Ki Num[^`]*`([^`]+)`', line)
        if m2:
            base_tier = m2.group(1).strip()
            base_ki = parse_ki(m2.group(2))
        continue
    if "| # Forma |" in line:
        in_table = True
        continue
    if in_table and line.strip().startswith("|") and not line.strip().startswith("| :-"):
        m = re_row.match(line)
        if m and current_char in targets and base_ki is not None:
            ...
            if form_tier in TIER_LIMITS:
                ...
                if form_tier not in ["1-C", "Low 1-C", "2-C", "Low 2-C", "2-B", "2-A", "High 3-A"]:
                    if calc_ki > t_max * 2:
                        print("{} | {} | tier={} | base_ki={} | mult={} | calc_ki={:.0f} | table_ki={:.0f} | max={} | ratio={:.1f}x".format(
                            current_char, name.strip()[:30], form_tier, base_ki, mult, calc_ki, ki_val, t_max, calc_ki/t_max))
