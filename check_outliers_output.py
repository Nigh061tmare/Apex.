...
for line in lines:
    ...
    if in_table and line.strip().startswith("|") and not line.strip().startswith("| :-"):
        m = re_row.match(line)
        if m and current_char and base_ki is not None:
...
print("FORM TIER MISMATCHES (calc Ki > 2x tier ceiling):")
print("=" * 100)
for o in outliers[:30]:
    print("{:35s} | {:35s} | {:10s} | base={:>12.0f} x{:.2f} = {:>15.0f} | ceil={:>15.0f} | ratio={:.2f}x".format(
        o['char'][:35], o['form'][:35], o['tier'], o['base_ki'], o['mult'], o['calc_ki'], o['ceiling'], o['ratio']))
print("=" * 100)
print("Total mismatches: {}".format(len(outliers)))